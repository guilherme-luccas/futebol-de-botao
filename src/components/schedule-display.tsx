...
export default function ScheduleDisplay({ schedule, onScoreChange, onDrawFields, numFields }: ScheduleDisplayProps) {
  ...
  // Agrupar rodadas em pares para o accordion
  const roundPairs = [];
  for (let i = 0; i < schedule.schedule.length; i += 2) {
    roundPairs.push(schedule.schedule.slice(i, i + 2));
  }

  const allRoundKeys = roundPairs.map((_, index) => `item-${index}`);
    ...
    <div className="space-y-4">
        ...
        <Accordion type="multiple" className="w-full space-y-4" value={openRounds} onValueChange={setOpenRounds}>
          {roundPairs.map((pair, pairIndex) => (
            <AccordionItem key={`pair-${pairIndex}`} value={`item-${pairIndex}`} className="border rounded-lg">
              <AccordionTrigger className="text-xl font-headline px-6 py-4 hover:no-underline">
                {pair.length === 2 
                  ? `Rodadas ${pair[0].round} e ${pair[1].round}`
                  : `Rodada ${pair[0].round}`
                }
              </AccordionTrigger>
              <AccordionContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
                  {pair.map((round, indexInPair) => {
                    const roundIndex = pairIndex * 2 + indexInPair;
                    return (
                      <Card key={round.round}>
                        <div className="px-6 py-4">
                          <h3 className="text-lg font-semibold mb-4">Rodada {round.round}</h3>
                          <div className="space-y-4">
